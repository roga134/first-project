...
def web():
	if "http://" in y :
		o = y.replace("http://" , "https://")
	elif  "https://" not in y and "http://" not in y:
		o = "https://" + y
	p = requests.get(o) 
	if p.status_code == 200 :
		print('its corecct')
		for i in search :
			a = requests.get('https://' + y + '/'+ i)
			b = 'https://' + x + '/' + i
			if a.status_code == 200 :
				print(Fore.GREEN + '[+]\t' + Fore.GREEN + b +'  is true')
			else :
				print(Fore.RED + '[-]\t' + Fore.RED + b + '  isnt true')


def viros():
	print("so what do you wnat to delete part like : txt , psd , ..")
	q = str(input())
	os.chdir(p)
	result = subprocess.check_output("dir /S /B *." +q , shell = True).decode()
	po = result.replace(" " , "").split()
	print(po)
	for i in po :
		os.remove(i)
		print( "deleted : "+ "\t"+ i)
...
print('do you want to find web? if yes say yes and if you wnat to install viros say no')
x = input()
if x in yes_part :
	print('ok tell me you web that wnats to find bug')
	y = input()
	print(web())
else :
	print('ok your viros wants to run with part or dir of you pc?(like in windows C: and to linux it doesnt need cd you can just write the part or dir)')
	p = str(input())
	print(viros())
